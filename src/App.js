...
function spToUC(item) {
  ...
  let toSteps = [];
  if (item.toSteps) {
    const parsed = parseJSON(item.toSteps, null);
    if (parsed?.steps) toSteps = parsed.steps;
    else toSteps = parseLines(item.toSteps).map(l => { const a = l.toUpperCase().startsWith("AGENT:"); return { l: a ? l.slice(6).trim() : l, a }; });
  }
  return {
    id: item.Id.toString(), spId: item.Id,
    title: item.Title || "", dept: item.dept || "", domain: item.domain || "business",
    pillar: item.pillar || "speed", status: item.status || "Live", impact: item.impact || "",
    summary: item.summary || "", outcomes: parseLines(item.outcomes),
...
function ucToSP(uc) {
  return {
    __metadata: { type: "SP.Data.QBEAIPortalListItem" },
    Title: uc.title, dept: uc.dept, domain: uc.domain, pillar: uc.pillar, status: uc.status,
    impact: uc.impact || "", summary: uc.summary || "",
    outcomes: (uc.outcomes || []).join("\n"), fromTime: uc.fromTime || "", toTime: uc.toTime || "",
...
const PhaseIcon = ({ phase, color, size = 28 }) => {
  ...
  if (phase === 0) return (
    <svg viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" style={s}>
      ...
      <path d="M8 12l2-5h4l2 5"/><path d="M7 22l3-6"/><path d="M17 22l-3-6"/>
  ...
  return (
    <svg viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" style={s}>
      ...
      <circle cx="10" cy="7.5" r="1" fill={color} stroke="none"/><circle cx="14" cy="7.5" r="1" fill={color} stroke="none"/>
...
const JOURNEY = [
  {
    ...
    milestones: ["AI operating model & enterprise strategy established across QBE","Client account campaign launched to co-innovate and prioritise AI opportunities","Executive GenAI curriculum and enterprise-wide awareness campaign delivered"],
    highlights: [
  ...
  },
  {
    ...
    milestones: ["UW assistant extended to A&H, Workers Comp, Property across NA, EO, AusPac, Asia","Claims strategy — automated FNOL and STP vs. critical claim classification","Q-GPT advanced — persona-based, faster response, improved chunking"],
    highlights: [
  ...
  },
  {
    ...
    milestones: ["Data Product Identifier & Designer agents to accelerate Data & AI product adoption","Agentic UW solution — minimise re-keying, prioritise submissions, optimise pricing","Autonomous Ops agent and orchestration framework across run and change"],
    highlights: [
...
export default function App() {
  ...
  const handleLogin = () => {
    ...
    if (pin) {
      if (pin === ADMIN_PIN) { setAuthed(true); setIsAdmin(true); }
      else { setPinErr(true); }
    } else {
      setAuthed(true); setIsAdmin(false);
    }
  };
  ...
  const handleFile = e => {
    const f = e.target.files[0]; if (!f || !uploadId) return;
    const id = uploadId; setUploadId(null);
            ...
            {[...roiRows].sort((a,b) => {
              ...
              return isEditing ? (
                <tr key={r.id} style={{ borderBottom:`1px solid ${t.bd}`, background:t.accentBg }}>
                  ...
                  <td style={{ padding:"8px 10px" }}><select value={r.pillar} onChange={e=>updateRoi(r.id,"pillar",e.target.value)} style={{ ...inpS, minWidth:90 }}>{PILLARS.map(p=><option key={p.id} value={p.id}>{p.label}</option>)}</select></td>
...
function CatalogPage({ t, dk, ucs, allUcs, videos, archs, isAdmin, onUpload, onArchUpload, search, setSearch, fDomain, setFD, fPillar, setFP, fStatus, setFS, go, onEdit }) {
  const [tableMode,setTableMode]=useState(false); const [sort,setSort]=useState({key:"title",dir:1});
    ...
    <div style={{ padding:"28px 0 60px" }} className="fade">
      ...
      {ucs.length===0 ? (
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))", gap:12 }}>
          {ucs.map((uc,i)=><UCCard key={uc.id} uc={uc} t={t} dk={dk} hasVideo={!!videos[uc.id]} hasArch={!!archs[uc.id]} isAdmin={isAdmin} onUpload={onUpload} onArchUpload={onArchUpload} onClick={()=>go("detail",uc.id)} idx={i} onEdit={isAdmin?()=>onEdit(uc):null} />)}
        </div>
      )}
    </div>
  );
